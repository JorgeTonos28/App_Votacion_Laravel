...
<div class="admin-shell {{ $isEventContext?'is-event-context':'' }}">
    <header class="admin-topbar" aria-label="Contexto administrativo"><button class="icon-button menu-toggle" type="button" data-menu-toggle aria-label="Abrir menú"><span class="material-symbols-outlined">menu</span></button><a class="topbar-logo" href="{{ route('admin.index') }}"><span class="material-symbols-outlined">leaderboard</span><strong>{{ $isEventContext?trim($__env->yieldContent('title')):'InnovaMente' }}</strong></a><div class="admin-user"><a href="{{ route('admin.mfa') }}" title="Seguridad de la cuenta">{{ auth()->user()->email }}</a><span class="avatar"><span class="material-symbols-outlined">person</span></span></div></header>
    ...
    <main class="admin-content @yield('admin-content-class')">
        @if($isEventContext)<div class="event-context-banner"><div><span class="material-symbols-outlined">event</span><strong>{{ $event->name }}</strong><code>{{ $event->code }}</code></div></div>@endif
        @yield('content')
    </main>
</div>
